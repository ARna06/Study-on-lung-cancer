...
\pgfdeclarelayer{image}
\pgfsetlayers{bg,image,main,fg}  % set the order of the layers (main is the standard layer)
...
\begin{pgfonlayer}{image}
     \node[opacity=0.9,inner sep=0pt] at (current page.center) {\includegraphics[width=\paperwidth,height=\paperheight]{images/cover.png} 
  };
\end{pgfonlayer}
...
\begin{pgfonlayer}{fg}
    ...
    % AUTHOR + DATE + SUBJECT
    \drawtext%
    {{\color{fgalt}Prepared by: }\textsc{\textbf{#3}}\\{\color{fgalt}Date: }\textsc{\textbf{#4}}\\{\color{fgalt}Source code at: }\textsc{\textbf{#5}}}%
